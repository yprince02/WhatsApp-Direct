...
const Home = () => {
  ...
  const [copyText, setCopyText] = useState('Copy Link To Share');
  ...
  let savePhone = async phone => {
    ...
    fetch(
      'https://whatsapp-direct-413f6-default-rtdb.firebaseio.com/phone-numbers.json',
      requestOptions,
    )
    ...
    <SafeAreaView>
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.heading}>
          WhatsApp<Text style={{color: ColorCodes.Blue}}>-Direct Pro</Text>
        </Text>
        ...
        <View style={{flexDirection: 'row'}}>
          ...
          <TextField
            ...
            onChangeText={text => (
              setPhone(text), setCopyText('Copy Link To Share')
            )}
          />
        </View>
        <TextField
        ...
        <TouchableOpacity
          activeOpacity={0.6}
          style={styles.copyLinkC}
          onPress={() => CopyLink(phone, message)}>
          <Image
            style={{width: 15, height: 15}}
            source={{uri: 'https://i.ibb.co/xfJGcwx/image.png'}}
          />
